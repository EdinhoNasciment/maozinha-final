package com.maozinha.api.config;
...
import com.maozinha.api.security.SystemUser;
...
public class AuthorizationServerConfig {
	...
	RegisteredClientRepository registeredClientRepository() {
		List<String> allowedRedirects = Arrays.asList("http://local-maozinha.arq.ifsp.edu.br:8000/authorized","https://oidcdebugger.com/debug");
		...
		RegisteredClient angularClient = RegisteredClient.withId(UUID.randomUUID().toString()).clientId("angular")
				...
				.redirectUris(uris -> uris.addAll(allowedRedirects)).scope("read").scope("write")
		...
		RegisteredClient mobileClient = RegisteredClient.withId(UUID.randomUUID().toString()).clientId("mobile")
				...
				.authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE)
				.authorizationGrantType(AuthorizationGrantType.REFRESH_TOKEN)
				.redirectUris(uris -> uris.addAll(allowedRedirects)).scope("read")
				.tokenSettings(TokenSettings.builder().accessTokenTimeToLive(Duration.ofMinutes(30))
						.refreshTokenTimeToLive(Duration.ofDays(30)).build())
